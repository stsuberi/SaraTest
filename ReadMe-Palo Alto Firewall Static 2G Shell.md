...
|Send Custom Command|Executes a custom command on the device. <br>Command Inputs:* **Command**: CLI command to run on the resource.</br>|
|Save|Creates a configuration file and saves it to the provided destination.<br>Command Inputs:</br>* **Folder Path**: Path to the configuration file. The file must be accessible to the execution server that will run the traffic test.<br>* **Configuration Type**: xxx.  </br>|
